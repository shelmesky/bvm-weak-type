...
type Node struct {
	...
	Line   int         // 行
	Column uint32      // 列
	Result uint32      // 如果节点是表达式, 则是表达式的类型
	Value  interface{} // 节点所包含的子节点, 或者值(如果是字面值)
}
...
func newVarExp(name string, exp *Node, l yyLexer) []NVar {
	va := make([]NVar, 1)
	va = []NVar{
		{
			Name: name,
...
func newVarDecl(vars []string, l yyLexer) *Node {
...
func newFunc(name string, pars []NVar, body *Node, l yyLexer) *Node {
	return setPos(&Node{
		Type: TFunc,
		Value: &NFunc{
			Name:   name,
